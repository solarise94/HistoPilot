...
def test_owner_bootstrap_create_and_reset(monkeypatch):
    ...
    check("owner email 用 ADMIN_USERNAME", owner["email"] == "browser_admin")
    check("owner 可登录", user_store.verify_user("browser_admin", "owner-pass") is not None)
    ...
    check("旧密码失效", user_store.verify_user("browser_admin", "owner-pass") is None)
    check("新密码生效", user_store.verify_user("browser_admin", "new-owner-pass") is not None)
...
def test_login_success_sets_role(monkeypatch):
    ...
    client = make_client()
    r = login(client, "browser_admin", "owner-pass")
    check("owner 登录 302", r.status_code == 302)
    with client.session_transaction() as s:
        check("session auth_user 为 display_name", s.get("auth_user") == "browser_admin")
...
def test_admin_users_owner_vs_user(monkeypatch):
    ...
    client = make_client()
    # owner 登录
    login(client, "browser_admin", "owner-pass")
...
def test_disable_invalidates_existing_session(monkeypatch):
    ...
    login(owner_client, "browser_admin", "owner-pass")
