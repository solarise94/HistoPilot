 ...
 *
 * Directory layout (under `<sessionsDir>`, env HISTOPILOT_SESSIONS_DIR;
 * default ~/.histopilot/sessions). It is never shared with platform data.
 ...
 *
 * Priority:
 *   1. `HISTOPILOT_SESSIONS_DIR` env.
 *   2. Legacy `AI_SESSIONS_DIR` env.
 *   3. `~/.histopilot/sessions`.
 */
function defaultSessionsDir(): string {
	const env = process.env.HISTOPILOT_SESSIONS_DIR || process.env.AI_SESSIONS_DIR;
	if (env && env.trim()) return env.trim();
	return join(homedir(), ".histopilot", "sessions");
}
...
export interface SessionStoreOptions {
	/** Override sessions directory. Defaults to HISTOPILOT_SESSIONS_DIR. */
	sessionsDir?: string;
