...
/**
 * Resolve the service token from the new HistoPilot env, with the old name kept
 * as a migration alias. A config-dir token file is accepted for local setups.
 */
export async function resolveAiInternalToken(): Promise<string> {
	const envTok = process.env.HISTOPILOT_INTERNAL_TOKEN || process.env.AI_INTERNAL_TOKEN;
	...
	const tok = raw.trim();
	if (!tok) {
		throw new Error(`HISTOPILOT_INTERNAL_TOKEN not set and ${p} is empty`);
	}
	return tok;
}
...
export interface FlaskClientOptions {
	/** PathTogether URL (env PATHTOGETHER_URL). */
	baseUrl?: string;
...
export class FlaskClient {
	...
	constructor(opts: FlaskClientOptions = {}) {
		this.baseUrl = (opts.baseUrl ?? process.env.PATHTOGETHER_URL ?? process.env.AI_FLASK_URL ?? "http://127.0.0.1:8000").replace(/\/+$/, "");
...
export async function createFlaskClient(opts: Omit<FlaskClientOptions, "token"> = {}): Promise<FlaskClient> {
	const token = process.env.HISTOPILOT_INTERNAL_TOKEN?.trim()
		|| process.env.AI_INTERNAL_TOKEN?.trim()
		|| (await readTokenFile());
