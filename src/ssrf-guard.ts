 ...
 * PathTogetherHttpClient 在 `withTrustedCallback` 内、且请求精确 origin 匹配
 * `PATHTOGETHER_URL` 时放行。
 */
...
function flaskBaseUrl(): string {
	return (process.env.PATHTOGETHER_URL || process.env.AI_FLASK_URL || "http://127.0.0.1:8000").replace(/\/+$/, "");
}
