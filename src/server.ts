 ...
 *
 * node:http server bound to `HISTOPILOT_HOST:HISTOPILOT_PORT`.
...
export interface SidecarServerOptions {
	...
	host?: string;
	/** Bind port (default env HISTOPILOT_PORT or 8055). */
	port?: number;
	 ...
	 * set HISTOPILOT_INTERNAL_TOKEN; index.ts honors HISTOPILOT_ALLOW_UNAUTH=1.
	 */
	allowUnauth?: boolean;
}
...
function defaultPort(): number {
	const p = parseInt(process.env.HISTOPILOT_PORT || process.env.AI_SIDECAR_PORT || "", 10);
...
export function assertInboundAuthAllowed(host: string, token: string, allowUnauth = false): void {
	...
	throw new Error(
		`HISTOPILOT_HOST=${host} is not loopback and inbound token is unset; refusing to start. Set HISTOPILOT_INTERNAL_TOKEN or bind 127.0.0.1`,
	);
}
