 ...
 *     `histopilot:{session_id}:{slide_fingerprint}:g{generation}` form, whose
 ...
 *
 * Form: `histopilot:{session_id}:{slide_fingerprint}:g{generation}`
 *
...
export function buildPromptCacheKey(args: {
	...
	const raw = `histopilot:${args.sessionId}:${args.slideFingerprint}:g${args.generation}`;
