...
describe("ssrfGuardedFetch", () => {
	...
	const origFlask = process.env.PATHTOGETHER_URL;
	afterAll(async () => {
		if (origFlask === undefined) delete process.env.PATHTOGETHER_URL;
		else process.env.PATHTOGETHER_URL = origFlask;
	...
	it("does not exempt the Flask origin outside the trusted callback context", async () => {
		...
		process.env.PATHTOGETHER_URL = srv.url;
...
describe("dns.lookup under SSRF guard", () => {
	...
	it("does not exempt Flask hostname without trusted callback (same-host other port)", async () => {
		const prev = process.env.PATHTOGETHER_URL;
		process.env.PATHTOGETHER_URL = "http://localhost:8000";
		try {
			const err = await new Promise<NodeJS.ErrnoException | null>((resolve) => {
				withSsrfGuard(() => {
					dns.lookup("localhost", (e) => resolve(e));
			...
			if (prev === undefined) delete process.env.PATHTOGETHER_URL;
			else process.env.PATHTOGETHER_URL = prev;
		}
	});
