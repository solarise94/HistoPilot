...
describe("SessionStore: sessions dir resolution (Stage 4-3)", () => {
	it("HISTOPILOT_SESSIONS_DIR takes priority over default", async () => {
		const prev = process.env.HISTOPILOT_SESSIONS_DIR;
		const custom = join(rootTmp, "custom-sessions");
		process.env.HISTOPILOT_SESSIONS_DIR = custom;
		try {
			const store = new SessionStore();
			expect(store.sessionsDir).toBe(custom);
		} finally {
			if (prev === undefined) delete process.env.HISTOPILOT_SESSIONS_DIR;
			else process.env.HISTOPILOT_SESSIONS_DIR = prev;
		}
	});

	it("falls back to ~/.histopilot/sessions when HISTOPILOT_SESSIONS_DIR unset", async () => {
		const prev = process.env.HISTOPILOT_SESSIONS_DIR;
		const prevHome = process.env.HOME;
		const fakeHome = join(rootTmp, "fake-home");
		delete process.env.HISTOPILOT_SESSIONS_DIR;
		process.env.HOME = fakeHome;
		try {
			const store = new SessionStore();
			expect(store.sessionsDir).toBe(join(fakeHome, ".histopilot", "sessions"));
		} finally {
			if (prev === undefined) delete process.env.HISTOPILOT_SESSIONS_DIR;
			else process.env.HISTOPILOT_SESSIONS_DIR = prev;
	...
	it("explicit sessionsDir option still wins over env", async () => {
		const prev = process.env.HISTOPILOT_SESSIONS_DIR;
		process.env.HISTOPILOT_SESSIONS_DIR = join(rootTmp, "env-dir");
		try {
