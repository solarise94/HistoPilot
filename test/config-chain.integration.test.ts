 ...
 *
 * The Flask-driven case reads a sibling PathTogether checkout, spawning its
 * Python (`.venv/bin/python`, falling back to `python3`) with an isolated
 * SHARE_DATA_DIR. `PATHTOGETHER_REPO` overrides the sibling path in CI. When
 * the platform checkout is absent only the cross-repository cases are skipped;
 * the pure HistoPilot cases always run.
 */
...
/** Independent PathTogether checkout that owns the Flask config producer. */
const PATHTOGETHER_REPO = process.env.PATHTOGETHER_REPO
	? resolve(process.env.PATHTOGETHER_REPO)
	: resolve(__dirname, "..", "..", "PathTogether");
const HAS_PATHTOGETHER = existsSync(join(PATHTOGETHER_REPO, "app.py"));
const VENV_PYTHON = join(PATHTOGETHER_REPO, ".venv", "bin", "python");
...
function findPython(): string | null {
	if (!HAS_PATHTOGETHER) return null;
	const candidates = [process.env.PATHTOGETHER_PYTHON, VENV_PYTHON, "python3", "python"]
		.filter((value): value is string => Boolean(value));
	for (const candidate of candidates) {
		if (candidate === VENV_PYTHON && !existsSync(candidate)) continue;
		try {
			// A version probe is insufficient on macOS, where /usr/bin/python3 may
			// exist without the platform dependencies. Verify the actual import.
			execFileSync(candidate, ["-c", "import flask, PIL, requests, openslide"], { stdio: "ignore" });
			return candidate;
		} catch {
			// Try the next interpreter on PATH.
		}
	}
	return null;
}
...
function buildFlaskSidecarConfig(): Record<string, unknown> {
	...
	const stdout = execFileSync(PYTHON, ["-c", BUILD_SIDECAR_CONFIG_SCRIPT, PATHTOGETHER_REPO], {
...
function flaskValidateAndBuild(body: Record<string, unknown>): Record<string, unknown> {
	if (!PYTHON) throw new Error("no python available");
	const tmpDir = mkdtempSync(join(tmpdir(), "svs-config-chain-"));
	const stdout = execFileSync(PYTHON, ["-c", VALIDATE_AND_BUILD_SCRIPT, PATHTOGETHER_REPO], {
...
function flaskSeedAndBuild(seed: Record<string, unknown>): Record<string, unknown> {
	if (!PYTHON) throw new Error("no python available");
	const tmpDir = mkdtempSync(join(tmpdir(), "svs-config-chain-"));
	const stdout = execFileSync(PYTHON, ["-c", SEED_AND_BUILD_SCRIPT, PATHTOGETHER_REPO], {
...
// =========================================================================== //
describe("HistoPilot tier resolution (pure TS)", () => {
