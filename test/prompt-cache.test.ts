...
describe("buildPromptCacheKey (§5.1/§10)", () => {
	it("produces the documented histopilot:{session}:{fp}:g{gen} form", () => {
		const key = buildPromptCacheKey({
			sessionId: "sess_abc",
			slideFingerprint: "fp-test:1234",
			generation: 3,
		});
		expect(key).toBe("histopilot:sess_abc:fp-test:1234:g3");
	});
	...
	it("truncates to PROMPT_CACHE_KEY_MAX_LENGTH (OpenAI limit)", () => {
		...
		expect(key.startsWith("histopilot:")).toBe(true);
	});
});
...
describe("buildCacheKeySamplingParams (explicit injection)", () => {
	it("returns the field under the documented OpenAI name for explicit mode", () => {
		...
		const params = buildCacheKeySamplingParams(caps, "histopilot:s1:fp1:g1");
		expect(params).not.toBeNull();
		expect(params![PROMPT_CACHE_KEY_FIELD]).toBe("histopilot:s1:fp1:g1");
	});
