...
测试在 `test/experiments-*.test.ts`（vitest，含 `experiments-runner.test.ts`）。
...
.venv/bin/python experiments/fixtures/generate.py

# 生成 + pin manifest（需要一个可访问、且已部署生成切片的 Flask）
.venv/bin/python experiments/fixtures/generate.py --pin \
    --flask-url http://127.0.0.1:5000 \
    --manifest experiments/fixtures/manifest.json
```
